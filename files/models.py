...
class Product(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        self.color_model = (download_file.color_mode())  # Цветовая модель
        logger.info(f"[INFO] Цветовая модель: {self.color_model}")
        self.width, self.length, self.resolution = download_file.check_tiff()
        logger.info(f"[INFO] Разрешение: {self.resolution}")
